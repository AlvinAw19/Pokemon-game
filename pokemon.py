...
def get_all_pokemon_types() -> ArrayR[Pokemon]:
    """ 
    Gets all Pokemon's class.

    Returns: 
        An ArrayR containing the class of all Pokemon.
        
    Complexity: 
    Best Case:  For each subclass, it assigns the class to an element in the all_pokemon array.
                The number of iterations depends on the number of subclasses, N.
                Therefore, the total complexity is O(N).
    Worst Case: Same as the Best Case.
    """
...
def get_all_pokemon_healths() -> ArrayR[int]:
    """ 
    Gets all Pokemon's health.

    Returns: 
        An ArrayR containing the health of all Pokemon.
        
    Complexity: 
    Best Case:  The number of iterations depends on the number of subclasses, N.
                Instantiating each class and getting the health attribute both have constant time complexity (O(1)).
                Therefore, the total complexity is O(N).
    Worst Case: Same as the Best Case
    """
    all_pokemon_healths = ArrayR(77)
    i = 0
    for name, cls in inspect.getmembers(inspect.getmodule(inspect.currentframe()), inspect.isclass):
        if cls != Pokemon and issubclass(cls, Pokemon):
            pokemon_instance = cls()  # Instantiate the class
            all_pokemon_healths[i] = pokemon_instance.health  # Append the health attribute
            i += 1
    return all_pokemon_healths
